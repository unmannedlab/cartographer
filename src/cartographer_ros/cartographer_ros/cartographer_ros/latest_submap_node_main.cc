...
private:
    ...
    ::ros::Publisher latest_submap_publisher_ GUARDED_BY(mutex_);
    std::map<SubmapId, SubmapSlice> submap_slices_ GUARDED_BY(mutex_);
    ::ros::WallTimer latest_submap_publisher_timer_;
...
Node::Node(const double resolution, const double publish_period_sec)
    : resolution_(resolution),
      ...
      latest_submap_publisher_(
          node_handle_.advertise<::nav_msgs::OccupancyGrid>(
              "latest_submap", kLatestOnlyPublisherQueueSize,
              true /* latched */)),
      latest_submap_publisher_timer_(
    ...
    const cartographer_ros_msgs::SubmapList::ConstPtr &msg)
{
    ...
    if (latest_submap_publisher_.getNumSubscribers() == 0)
    {
        return;
    }
    ...
    const auto &submap_msg = msg->submap.back();
...
void Node::DrawAndPublish(const ::ros::WallTimerEvent &unused_timer_event)
{
    ...
    latest_submap_publisher_.publish(*msg_ptr);
}
...
int main(int argc, char **argv)
{
    ...
    ::ros::init(argc, argv, "latest_submap_node");
    ::ros::start();
