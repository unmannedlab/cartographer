...
class Node {
 ...
 private:
  ...
  ::ros::Publisher multi_submaps_publisher_ GUARDED_BY(mutex_);
  std::map<SubmapId, SubmapSlice> submap_slices_ GUARDED_BY(mutex_);
  ::ros::WallTimer multi_submaps_publisher_timer_;
...
Node::Node(const double resolution, const double publish_period_sec)
    : resolution_(resolution),
      ...
      multi_submaps_publisher_(
          node_handle_.advertise<::nav_msgs::OccupancyGrid>(
              "multi_submaps", kLatestOnlyPublisherQueueSize,
              true /* latched */)),
      multi_submaps_publisher_timer_(
...
void Node::HandleSubmapList(
  ...
  if (multi_submaps_publisher_.getNumSubscribers() == 0) {
    return;
  }
  ...
  int submap_size = msg->submap.size();
  int count = 2;
  int thred = submap_size<=count?submap_size:count;

  submap_slices_.clear();
  for (int i =0;i<thred;i++) {
    const auto& submap_msg = msg->submap[submap_size-i-1];
...
void Node::DrawAndPublish(const ::ros::WallTimerEvent& unused_timer_event) {
  ...
  LOG(INFO) << "Multi submaps size: " << submap_slices_.size();
  ::cartographer::common::MutexLocker locker(&mutex_);
  auto painted_slices = PaintSubmapSlices(submap_slices_, resolution_);
  std::unique_ptr<nav_msgs::OccupancyGrid> msg_ptr = CreateOccupancyGridMsg(
      painted_slices, resolution_, last_frame_id_, last_timestamp_);
  multi_submaps_publisher_.publish(*msg_ptr);
}
...
int main(int argc, char** argv) {
  ...
  ::ros::init(argc, argv, "multi_submaps_node");
  ::ros::start();
